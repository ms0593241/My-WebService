...
<body>
   ...
   console.log("str: " + str);
	...
	function getCheckboxValue(value) {
		...
		var postToCheckboxStr = "<button type='button' class='btn-primary i18n' style='position:relative; left:500px; top: 5px; ' onclick=\"window.open ('"+ buttonURL +"','new window','height=300,width=400,top=250,left=550, toolbar =no, menubar=no, scrollbars=no, resizable=no, location=no, status=no')\" name='PickAttributes'></button>";
		...
		postToCheckboxStr += "<button type='button' class='btn-default i18n' style='position:relative; left:600px; top: 5px; ' onclick='location.reload()' name='reload'>還原</button>";
		...
		function doDemo() {
			for (var k = 0; k <= dataKeyList.length; k++) {
				if (k == 0) {
					headtableStr = "<th id='" + keyWord + "'><table><tbody><tr><td id='" + keyWord + "' width=''><a href='javascript:sortTable("
					+ k
					+ ")'  class='i18n' name='" + keyWord + "'></a></td></tr></tbody></table></th>";
					
				}else{
					headtableStr += "<th id='" + dataKeyList[k-1] + "'><table><tbody><tr><td id='" + dataKeyList[k-1] + "' width=''><a href='javascript:sortTable("
						+ k
						+ ")'  class='i18n' name='" + dataKeyList[k-1] + "''></a></td></tr></tbody></table></th>";
